...
def folder_initialization(date: str, algorithms_names: list, Ms: list):
    assert os.path.exists(os.path.join('Execution_Outputs'))

    folders = ['Logs', 'Csv', 'Plot']
    ...
    for index_folder, folder in enumerate(folders):
        ...
        if index_folder >= 1:
            for algorithm_name in algorithms_names:
                for M in [0] if algorithm_name == 'FPD' else Ms:
                    make_folder(os.path.join(path, folder, algorithm_name + ('' if algorithm_name == 'FPD' else '_{}'.format(M))))


def log_files_initialization(date: str, algorithms_names: list, Ms: list):
    for algorithm_name in algorithms_names:
        for M in [0] if algorithm_name == 'FPD' else Ms:
            log_file = open(os.path.join('Execution_Outputs', date, 'Logs', '{}.txt'.format(algorithm_name + ('' if algorithm_name == 'FPD' else '_{}'.format(M)))), 'w')
            log_file.close()


def write_in_log_file(date: str, algorithm_name: str, problem: ExtendedProblem, n: int, info: dict):
    log_file = open(os.path.join('Execution_Outputs', date, 'Logs', '{}.txt'.format(algorithm_name)), 'a')
    log_file.write('Problem: ' + problem.__name__ + '    N: ' + str(n) + 
                   '    Ni: ' + str(info['Ni']) + '    T: ' + str(info['T']) + '    Nf: ' + str(info['Nf']) + '    Ng: ' + str(info['Ng']) + 
                   '    n_pp: ' + str(info['n_pp']) + '    ma: ' + str(info['ma']) + '\n')
    log_file.close()


def write_results_in_csv_file(p_list: np.array, f_list: np.array, date: str, algorithm_name: str, problem: ExtendedProblem, export_pareto_solutions: bool):
