...
class ExtendedProblem(Problem, ABC):
    ...
    def filtered_lb_for_ini(self, filtered_lb_for_ini: np.array):
        ...
        assert not np.isinf(np.sum(filtered_lb_for_ini))
    ...
    def filtered_ub_for_ini(self, filtered_ub_for_ini: np.array):
        ...
        assert not np.isinf(np.sum(filtered_ub_for_ini))

        self.__filtered_ub_for_ini = filtered_ub_for_ini

    def set_lb(self, lb: np.array):
        assert np.allclose(self.filtered_lb_for_ini, lb)
        self.lb = lb

    def set_ub(self, ub: np.array):
        assert np.allclose(self.filtered_ub_for_ini, ub)
        self.ub = ub
